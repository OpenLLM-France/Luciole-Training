...
def convert_messages(data: DocumentsPipeline, rank: int = 0, world_size: int = 1, tokenizer=None) -> DocumentsPipeline:
    ...
    for document in data:
        domain = document.metadata.get("capability_target", "unknown")
...
if __name__ == "__main__":
    ...
    dataset_name = "nemotron_posttraining_v3_chat"
    ...
    pipeline = [
        ...
        #),
        JsonlReader(
            "hf://datasets/nvidia/Nemotron-Instruction-Following-Chat-v1/data",
            glob_pattern="*.jsonl",
            #adapter=instruct_adapter,
            text_key="messages",
        ),
