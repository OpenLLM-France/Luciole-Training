...
def convert_messages(data: DocumentsPipeline, rank: int = 0, world_size: int = 1, keep_thinking=False, language="en") -> DocumentsPipeline:
    for document in data:
        ...
        problem_name = random.choice({
            ...
            "fr": ["Problème :", "Question :", "Requête :", "Demande :", "user:", ""],
            "es": ["Problema:", "Pregunta:", "Consulta:", "Solicitud:", "user:", ""],
...
if __name__ == "__main__":
    ...
    parser.add_argument("--subset", type=str, default="math", help="Subset to process", choices=["stem", "chat", "code", "math", "multilingual_fr", "multilingual_es", "multilingual_it", "multilingual_de"])
    parser.add_argument("--keep_thinking", action="store_true", help="Keep the thinking in the output", default=False)
    ...
    pipeline = [
        ParquetReader(
            "hf://datasets/nvidia/Nemotron-Post-Training-Dataset-v2/data", glob_pattern = args.subset +"*.parquet",
            text_key="category",
            id_key="uuid",
        ),
        partial(convert_messages,
                keep_thinking=args.keep_thinking,
                language="en" if not args.subset.startswith("multilingual") else args.subset.split("_")[-1]
        ),
